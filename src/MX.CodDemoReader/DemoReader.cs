...
/// <summary>
/// Reads game-state configuration data from Call of Duty demo streams.
/// </summary>
public class DemoReader
{
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="demoStream" /> is <see langword="null" />.</exception>
    public DemoReader(Stream demoStream, GameVersion gameVersion)
    {
        ...
        // Skip first byte.
        if (_gameVersion is GameVersion.CallOfDuty4 or GameVersion.CallOfDuty5)
        {
            _ = _demoStream.Seek(1, SeekOrigin.Begin);
        }
    }
    ...
    private static Dictionary<string, string> SplitConfigString(string input, char seperator = '\\')
    {
    ...
    private void ReadFromStream(byte[] buffer, int count)
    {
        ...
        {
            throw new InvalidOperationException("Unable to read from demo stream");
        }
